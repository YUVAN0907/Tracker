...
Deletes all purchasedProductBatch, purchasedProductCase, vendorPurchaseOrder, vendorPurchaseItem, 
and warehouseEntry records, then updates ALL PO status to 'pending'.
"""
...
def clear_test_data():
    ...
    # Delete all warehouseEntry records
    try:
        result = execute_graphql("""
        mutation DeleteAllWarehouseEntries {
          warehouseEntry_deleteMany(all: true)
        }
        """)
        print("✓ Deleted all warehouseEntry records")
    except Exception as e:
        print(f"✗ ERROR deleting warehouseEntry: {e}")
